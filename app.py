...
st.markdown("""
    ...
    .radial-gauge {
        ...
        background: radial-gradient(closest-side, white 79%, transparent 80% 100%),
                    conic-gradient(#f87171 76%, #f1f5f9 0);
        display: flex;
    ...
    /* ======================================================== */
    /* EXPLICIT BUTTON & TAB STYLING TO ELIMINATE BLACK BARS   */
    /* ======================================================== */
    
    /* Primary Buttons (Selected / Action Buttons) */
    ...
    /* Secondary Buttons (Unselected Navigation Tabs & Sub-Actions) */
    ...
    /* High-Contrast Light HTML Table */
    .styled-table {
        width: 100%;
...
def capture_biometrics(duration=10):
    ...
    for f in range(total_frames):
        ...
        if len(green_signals) > 20 and f % 6 == 0:
            ...
            ax.plot(wave, color='#4f46e5', lw=2)
            ax.set_title("Forehead Capillary Spectrum", color='#0f172a', fontsize=9, weight='bold')
...
if not st.session_state.booted:
    ...
    with center_col:
        ...
        st.markdown("""
        <div class="pop-card" style="text-align: center; padding: 40px 30px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; text-align: left; font-size: 0.82rem; margin-bottom: 24px;">
                <div style="background: #f8fafc; padding: 12px 16px; border-radius: 12px; border: 1px solid #e2e8f0; color: #0f172a;">
                    ✓ Identity Services &nbsp;&nbsp;<b style="color: #16a34a;">Connected</b>
                </div>
                <div style="background: #f8fafc; padding: 12px 16px; border-radius: 12px; border: 1px solid #e2e8f0; color: #0f172a;">
                    ✓ Secure Data Layer &nbsp;&nbsp;<b style="color: #16a34a;">Connected</b>
                </div>
                <div style="background: #f8fafc; padding: 12px 16px; border-radius: 12px; border: 1px solid #e2e8f0; color: #0f172a;">
                    ✓ Welfare Analytics &nbsp;&nbsp;<b style="color: #16a34a;">Connected</b>
                </div>
                <div style="background: #f8fafc; padding: 12px 16px; border-radius: 12px; border: 1px solid #e2e8f0; color: #0f172a;">
                    ✓ AI Risk Engine &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b style="color: #16a34a;">Connected</b>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="text-align: center; color: #94a3b8; font-size: 0.75rem; margin-top: 20px;">
            Privacy-first • Welfare-first • Local Edge AI Runtime
        </div>
        """, unsafe_allow_html=True)
    st.stop()
...
if not st.session_state.authenticated:
    ...
    with login_col:
        ...
        st.markdown("""
        <div class="pop-card" style="text-align: center; padding: 34px 28px 24px 28px; margin-bottom: 12px;">
        ...
        st.markdown("""
        <div style="display: flex; justify-content: space-around; text-align: center; margin-top: 14px; font-size: 0.75rem; color: #64748b; font-weight: 600;">
...
if user_role == "Personnel":
    role_nav_options = ["Home", "Voluntary Wellness Check-in"]
...
# Navigation Bar
nav_cols = st.columns([1] * len(role_nav_options) + [1.5, 1.2])
...
st.markdown("""
<div style="background: #ffffff; border: 1px solid #e2e8f0; border-left: 5px solid #4f46e5; border-radius: 14px; padding: 12px 20px; margin-bottom: 22px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 12px rgba(148, 163, 184, 0.08);">
...
if user_role == "Personnel":
    ...
    if st.session_state.current_nav == "Home":
        ...
        with col_right:
            st.markdown("""
            <div class="pop-card" style="height: calc(100% - 20px);">
        ...
        col_form, col_telemetry = st.columns([1, 1])
        with col_form:
            ...
            st.markdown("<h3 style='font-size: 1rem; font-weight: 700; color: #0f172a; margin-bottom: 14px;'>2. Voluntary Self-Reported Wellness</h3>", unsafe_allow_html=True)
            ...
            subjective_distress = st.select_slider(
                ...
                value=user["subjective_fatigue"]
            )
            st.markdown('</div>', unsafe_allow_html=True)

        with col_telemetry:
            st.markdown('<div class="pop-card">', unsafe_allow_html=True)
            st.markdown("<h3 style='font-size: 1rem; font-weight: 700; color: #0f172a; margin-bottom: 4px;'>3. Optional Authorized Telemetry</h3>", unsafe_allow_html=True)
            ...
            if start_scan:
                ...
                fig_wave, ax_wave = plt.subplots(figsize=(5, 1.8))
                fig_wave.patch.set_facecolor('#ffffff')
                ax_wave.set_facecolor('#ffffff')
                ax_wave.plot(pulse_waveform[-150:], color='#4f46e5', lw=1.8)
                ax_wave.set_title("Extracted Optical Capillary Spectrum Waveform", color='#0f172a', fontsize=9, weight='bold')
                ax_wave.tick_params(colors='#64748b', labelsize=8)
                for sp in ax_wave.spines.values(): sp.set_color('#e2e8f0')
...
# ==================== 2. COMMANDER ROLE VIEWS ==================== #
    ...
    if st.session_state.current_nav == "Workload Grid":
        ...
        with c_left:
            ...
            cats = ["Balanced", "Moderate Load", "Fatigue Rotation"]
            counts = [3, 0, 2]
            fig_bar, ax_bar = plt.subplots(figsize=(5, 3.2))
            fig_bar.patch.set_facecolor('#ffffff')
            ax_bar.set_facecolor('#ffffff')
            bars = ax_bar.bar(cats, counts, color=['#10b981', '#f59e0b', '#ef4444'], width=0.45)
            ax_bar.set_ylabel("Personnel Count", color='#0f172a', fontsize=9, weight='bold')
            ax_bar.tick_params(colors='#334155', labelsize=9)
            for s in ax_bar.spines.values(): s.set_color('#e2e8f0')
...
# ==================== 3. WELFARE OFFICER ROLE VIEWS ==================== #
else:

    if st.session_state.current_nav == "Welfare Assessment Triage":
        st.markdown(f'<div style="font-size: 1.15rem; font-weight: 800; color: #0f172a; margin-bottom: 16px;">Unit Welfare Officer Decision Support | Identity: {user["display_id"]}</div>', unsafe_allow_html=True)

        w_left, w_right = st.columns([1.2, 1.8])

        with w_left:
            st.markdown("""
            <div class="pop-card" style="text-align: center; padding: 28px 20px;">
                <div style="font-size: 0.85rem; font-weight: 700; color: #64748b; margin-bottom: 16px;">AI WELFARE ASSESSMENT</div>
                <div class="radial-gauge">
                    <div class="gauge-val">76</div>
                    <div class="gauge-sub">/ 100</div>
                </div>
                <div style="margin-top: 18px;">
                    <span class="pill-badge-red">RISK STATUS: ELEVATED</span>
                </div>
                <div style="font-size: 0.75rem; color: #64748b; margin-top: 8px;">Prototype Welfare Risk Score: 76/100</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with w_right:
            st.markdown("""
            <div class="pop-card">
                <div style="font-size: 0.92rem; font-weight: 800; color: #0f172a; margin-bottom: 16px;">WHY WAS THIS FLAGGED? (EVIDENCE CHAIN)</div>
                <div style="display: flex; flex-direction: column; gap: 14px; font-size: 0.82rem;">
                    <div>
                        <div style="display: flex; justify-content: space-between; margin-bottom: 4px; font-weight: 600; color: #0f172a;">
                            <span>Extended interval since sanctioned leave</span>
                            <span style="color: #ef4444;">82 days</span>
                        </div>
                        <div style="background: #f1f5f9; height: 8px; border-radius: 4px;"><div style="background: #ef4444; width: 82%; height: 100%; border-radius: 4px;"></div></div>
                    </div>
                    <div>
                        <div style="display: flex; justify-content: space-between; margin-bottom: 4px; font-weight: 600; color: #0f172a;">
                            <span>Consecutive night-watch load</span>
                            <span style="color: #ef4444;">4 watches</span>
                        </div>
                        <div style="background: #f1f5f9; height: 8px; border-radius: 4px;"><div style="background: #f87171; width: 65%; height: 100%; border-radius: 4px;"></div></div>
                    </div>
                    <div>
                        <div style="display: flex; justify-content: space-between; margin-bottom: 4px; font-weight: 600; color: #0f172a;">
                            <span>Sleep deficit</span>
                            <span style="color: #d97706;">< 4.0 hrs/day logged</span>
                        </div>
                        <div style="background: #f1f5f9; height: 8px; border-radius: 4px;"><div style="background: #fbbf24; width: 55%; height: 100%; border-radius: 4px;"></div></div>
                    </div>
                    <div>
                        <div style="display: flex; justify-content: space-between; margin-bottom: 4px; font-weight: 600; color: #0f172a;">
                            <span>Optional telemetry signal</span>
                            <span style="color: #4f46e5;">Acoustic jitter elevated (3.4%)</span>
                        </div>
                        <div style="background: #f1f5f9; height: 8px; border-radius: 4px;"><div style="background: #6366f1; width: 35%; height: 100%; border-radius: 4px;"></div></div>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            with b1:
                if b1.button("🏖️ Priority Leave", use_container_width=True, type="primary"):
                    st.success("Expedited leave advisory generated and routed to Battalion HQ.")
            with b2:
                if b2.button("👥 Buddy System", use_container_width=True, type="primary"):
                    st.info("Designated peer buddy notified for an informal welfare check-in.")
            with b3:
                if b3.button("📞 Tele-Counseling", use_container_width=True, type="primary"):
                    st.success("Tele-counseling referral recommended — authorised welfare follow-up required.")
        ...
        <div class="pop-card">
            ...
            <p style="color: #64748b; font-size: 0.8rem; margin-bottom: 16px;">True local feature contributions computed dynamically from Random Forest tree paths for P-10001</p>
        """, unsafe_allow_html=True)

        p1 = PERSONNEL_REGISTRY["P-10001"]
        hardship_val = 1 if "Counter" in p1["deployment_hardship"] else (2 if "High" in p1["deployment_hardship"] else 0)
        sample_df = pd.DataFrame([{
            'Days_Since_Leave': p1["days_since_leave"],
            'Night_Shifts_7d': p1["night_shifts_7d"],
            'Deployment_Hardship': hardship_val,
            'Trauma_Incident_Flag': 1 if p1["trauma_incident_flag"] else 0,
            'Sleep_Hours': p1["sleep_hours"],
            'Subjective_Fatigue': p1["subjective_fatigue"],
            'Optional_Vocal_Jitter': 3.4,
            'Optional_BPM_Delta': 14.0
        }])

        if model is not None and explainer is not None:
            shap_values = explainer.shap_values(sample_df)
            if isinstance(shap_values, list):
                local_contribs = np.abs(shap_values[1][0])
            elif len(shap_values.shape) == 3:
                local_contribs = np.abs(shap_values[0, :, 1])
            else:
                local_contribs = np.abs(shap_values[0])
            ...
            ax_shap.bar(display_labels, pct_contribs, color='#4f46e5', width=0.5)
            ax_shap.set_ylabel("Contribution (%)", color='#0f172a', fontsize=9, weight='bold')
            ax_shap.tick_params(colors='#334155', labelsize=8)
            plt.xticks(rotation=25, ha='right')
            for s in ax_shap.spines.values(): s.set_color('#e2e8f0')
        ...
        <div class="pop-card">
            ...
            <p style="color: #64748b; font-size: 0.8rem; margin-bottom: 16px;">Tracking early risk indicators over rolling assessment periods for P-10001</p>
        """, unsafe_allow_html=True)
        
        days = ["01. Day 1", "02. Day 6", "03. Day 12", "04. Day 18", "05. Day 24", "06. Day 30"]
        scores = [32, 38, 48, 55, 64, 76]
        fig_trend, ax_trend = plt.subplots(figsize=(8, 3.2))
        fig_trend.patch.set_facecolor('#ffffff')
        ax_trend.set_facecolor('#ffffff')
        ax_trend.plot(days, scores, color='#ef4444', marker='o', lw=2.5, markersize=6)
        ax_trend.set_ylabel("Risk Score (0-100)", color='#0f172a', fontsize=9, weight='bold')
        ax_trend.set_ylim(0, 100)
        ax_trend.grid(color='#f1f5f9', linestyle='--', linewidth=1)
        ax_trend.tick_params(colors='#334155', labelsize=8)
        for s in ax_trend.spines.values(): s.set_color('#e2e8f0')
...
st.markdown("""
<div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 14px 24px; margin-top: 36px; display: flex; justify-content: space-between; align-items: center; font-size: 0.78rem; color: #64748b; box-shadow: 0 4px 14px rgba(148, 163, 184, 0.08);">
    <div>
